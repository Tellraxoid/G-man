...
@Dao interface TrainingDao {
    ...
    @Query("SELECT workout_sets.* FROM workout_sets INNER JOIN exercises ON exercises.id = workout_sets.exerciseId INNER JOIN workouts ON workouts.id = exercises.workoutId WHERE exercises.name = :name AND workouts.id = (SELECT w2.id FROM workouts w2 INNER JOIN exercises e2 ON e2.workoutId = w2.id WHERE e2.name = :name AND w2.endedAt IS NOT NULL ORDER BY w2.startedAt DESC LIMIT 1) ORDER BY workout_sets.createdAt, workout_sets.id") fun observePreviousExerciseSets(name:String):Flow<List<WorkoutSetEntity>>
