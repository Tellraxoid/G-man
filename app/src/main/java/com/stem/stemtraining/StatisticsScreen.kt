...
fun StatisticsScreen() {
    ...
    if(oneRm)OneRepMaxCalculator{oneRm=false}
}
...
@Composable private fun OneRepMaxCalculator(dismiss:()->Unit){var weight by remember{mutableStateOf("100")};var reps by remember{mutableStateOf("5")};val w=weight.replace(',','.').toDoubleOrNull()?:0.0;val r=reps.toIntOrNull()?:0;val result=estimatedOneRepMax(w,r);AlertDialog(onDismissRequest=dismiss,title={Text("Расчёт 1ПМ")},text={Column(verticalArrangement=Arrangement.spacedBy(10.dp)){OutlinedTextField(weight,{weight=it},label={Text("Вес, кг")});OutlinedTextField(reps,{reps=it.filter(Char::isDigit)},label={Text("Повторения")});HorizontalDivider();Text("Расчётный 1ПМ: ${formatStat(result)} кг",style=MaterialTheme.typography.headlineSmall,fontWeight=FontWeight.Bold);Text("Формула Epley: вес × (1 + повторения / 30). Для очень высоких повторений оценка становится менее точной.",style=MaterialTheme.typography.bodySmall)}},confirmButton={TextButton(dismiss){Text("Готово")}})}
