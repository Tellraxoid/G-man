...
fun StatisticsScreen() {
    ...
    LazyColumn(Modifier.padding(horizontal = 16.dp), verticalArrangement = Arrangement.spacedBy(12.dp)) {
        ...
        item{Card(Modifier.fillMaxWidth()){Column(Modifier.padding(18.dp)){Row(Modifier.fillMaxWidth(),horizontalArrangement=Arrangement.SpaceBetween){Text("Цель недели",style=MaterialTheme.typography.titleMedium);Text("$weekly / $goal",fontWeight=FontWeight.Bold)};LinearProgressIndicator(progress={(weekly.toFloat()/goal).coerceIn(0f,1f)},modifier=Modifier.fillMaxWidth().padding(vertical=10.dp));Text("Серия: $streak дн. подряд",color=MaterialTheme.colorScheme.secondary);Row(Modifier.fillMaxWidth().padding(top=8.dp),horizontalArrangement=Arrangement.spacedBy(8.dp)){OutlinedButton({plates=true},Modifier.weight(1f)){Text("Блины")};OutlinedButton({oneRmCalculator=true},Modifier.weight(1f)){Text("Калькулятор 1ПМ")}}}}}
    ...
    if(oneRmCalculator)OneRmCalculator{oneRmCalculator=false}
}
...
@Composable private fun OneRmCalculator(dismiss:()->Unit){
    ...
    val result=estimatedOneRepMax(w,r)
    AlertDialog(onDismissRequest=dismiss,title={Text("Расчёт 1ПМ")},text={Column(verticalArrangement=Arrangement.spacedBy(10.dp)){
        Text("Оценка максимального веса на одно повторение по формуле Epley.")
        OutlinedTextField(weight,{weight=it},label={Text("Вес подхода, кг")},singleLine=true)
        OutlinedTextField(reps,{reps=it.filter(Char::isDigit)},label={Text("Количество повторений")},singleLine=true)
        if(result>0)Surface(shape=MaterialTheme.shapes.medium,color=MaterialTheme.colorScheme.primaryContainer){Column(Modifier.fillMaxWidth().padding(16.dp)){Text("Расчётный 1ПМ",style=MaterialTheme.typography.labelLarge);Text("${formatStat(result)} кг",style=MaterialTheme.typography.headlineMedium,fontWeight=FontWeight.Bold)}}
        Text("Это ориентир, а не рекомендация проверять максимум без страховки.",style=MaterialTheme.typography.bodySmall,color=MaterialTheme.colorScheme.onSurfaceVariant)
    }},confirmButton={TextButton(dismiss){Text("Готово")}})
}
